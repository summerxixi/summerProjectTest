...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    __cs149_vec_float floatend = _cs149_vset_float(9.999999f);
    ...
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        ...
        _cs149_vload_float(x, values + i, maskAll); //复制给x
        _cs149_vload_int(y, exponents + i, maskAll); //复制给y
        ...
        maskResult = _cs149_init_ones(0);
        _cs149_vgt_float(maskResult, result, floatend, maskNotZero); //if result>9.999999f
        _cs149_vset_float(result, 9.99999f, maskResult); //result=9.999999f
